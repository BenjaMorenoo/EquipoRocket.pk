...
export default function AdminSimulationsAnalytics() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error('admin sim analytics error', e.message || e);
        setError(e.message || 'Error fetching sim metrics');
      } finally {
  ...
  if (loading) return <div className="pk-card" style={{ padding: 20 }}>Cargando métricas de simulaciones...</div>;
  if (error) return <div className="pk-card" style={{ padding: 20, color: '#ef4444' }}>Error: {error}</div>;
  if (!data || !data.length) return <div className="pk-card" style={{ padding: 20 }}>No hay métricas de simulaciones disponibles.</div>;
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      <div className="pk-card" style={{ padding: 12 }}>
        <details>
          <summary style={{ cursor: 'pointer', fontWeight: 700, marginBottom: 8 }}>Simulations — detalles y métricas</summary>
          <div style={{ marginTop: 8, color: 'var(--color-pk-muted)', fontSize: 13, lineHeight: 1.45 }}>
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li><strong>Éxito simulación (%)</strong>: tasa de éxito reportada por las simulaciones Monte Carlo asociadas al equipo.</li>
              <li><strong>Confianza combinada (%)</strong>: métrica interna que pondera simulaciones y feedback para estimar calidad.</li>
              <li>El gráfico combina feedback (+/-) y muestra la tasa de simulación como línea para comparar señales humanas vs técnicas.</li>
            </ul>
          </div>
        </details>
      </div>
      ...
      <div className="pk-card" style={{ padding: 12 }}>
        <h3 style={{ margin: '0 0 8px 0', fontFamily: 'var(--font-heading)', fontWeight: 700 }}>Simulaciones — Resumen</h3>
