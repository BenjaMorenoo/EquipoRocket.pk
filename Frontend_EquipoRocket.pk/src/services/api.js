...
// Frontend ONLY comunica con el gateway. El gateway routea a cada microservicio.
...
// ── Auth endpoints (→ gateway → ms_auth) ────────────────────────────────────
export const loginUser = async (credentials) => {
  const { data } = await gatewayAPI.post('/api/auth/login', credentials);
  return data;
};

export const registerUser = async (userData) => {
  const { data } = await gatewayAPI.post('/api/auth/register', userData);
  return data;
};

export const getCurrentUser = async () => {
  const { data } = await gatewayAPI.get('/api/auth/me');
  return data;
};

export const updateCurrentUser = async (payload) => {
  const { data } = await gatewayAPI.patch('/api/auth/me', payload);
  return data;
};

export const verifyCurrentPassword = async (password) => {
  const { data } = await gatewayAPI.post('/api/auth/verify-password', { password });
  return data;
};

// Admin: list users and toggle active state
export const getUsers = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/users');
  return data;
};
...
export const setUserActive = async (id, active, password = undefined) => {
  ...
  const { data } = await gatewayAPI.patch(`/api/usuarios/users/${id}/active`, body);
  return data;
};

// ── Teams endpoints (→ gateway → ms_usuarios) ───────────────────────────────
export const getTeams = async () => {
  const { data } = await gatewayAPI.get('/api/teams');
  return data;
};

export const getTeamById = async (id) => {
  const { data } = await gatewayAPI.get(`/api/teams/${id}`);
  return data;
};

export const createTeam = async (team) => {
  const { data } = await gatewayAPI.post('/api/teams', team);
  return data;
};

export const updateTeam = async (id, team) => {
  const { data } = await gatewayAPI.put(`/api/teams/${id}`, team);
  return data;
};

export const deleteTeam = async (id) => {
  const { data } = await gatewayAPI.delete(`/api/teams/${id}`);
  return data;
};

// ── User collections (→ gateway → ms_usuarios) ──────────────────────────────
export const getCollections = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/collections');
  return data;
};

export const addToCollection = async (pokemon_id) => {
  const { data } = await gatewayAPI.post('/api/usuarios/collections', { pokemon_id });
  return data;
};

export const removeFromCollection = async (pokemon_id) => {
  const { data } = await gatewayAPI.delete(`/api/usuarios/collections/${pokemon_id}`);
  return data;
};

// ── Pokemon endpoints (→ gateway → ms_pokemon) ──────────────────────────────
export const getBackendPokemons = async (limit = 200, offset = 0) => {
  const { data } = await gatewayAPI.get(`/api/pokemon?limit=${limit}&offset=${offset}`);
  return data;
};

export const getBackendPokemon = async (name) => {
  const { data } = await gatewayAPI.get(`/api/pokemon/${encodeURIComponent(name)}`);
  return data;
};

// ── Data lists (→ gateway → ms_usuarios) ────────────────────────────────────
export const getMovesList = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/data/moves');
  return data;
};

export const getAbilitiesList = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/data/abilities');
  return data;
};

export const getItemsList = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/data/items');
  return data;
};

export const getSpreadsList = async (pokemonName) => {
  const url = `/api/usuarios/data/spreads${pokemonName ? `?pokemon=${encodeURIComponent(pokemonName)}` : ''}`;
  const { data } = await gatewayAPI.get(url);
  return data;
};

export const createSpread = async ({ nature, ev }) => {
  const { data } = await gatewayAPI.post('/api/usuarios/data/spreads', { nature, ev });
  return data;
};

// ── Team feedback (→ gateway → ms_usuarios) ─────────────────────────────────
export const postTeamFeedback = async (team_id, type) => {
  const { data } = await gatewayAPI.post(`/api/teams/${team_id}/feedback`, { type });
  return data;
};

export const getTeamFeedback = async (team_id) => {
  try {
    const { data } = await gatewayAPI.get(`/api/teams/${team_id}/feedback`);
    return data;
  } catch (err) {
...
// ── Admin metrics (→ gateway → ms_usuarios) ─────────────────────────────────
export const getAdminTeamPerformance = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/admin/teams/performance');
  return data;
};

export const getTypesByCountry = async () => {
  const { data } = await gatewayAPI.get('/api/usuarios/admin/usage/types-by-country');
  return data;
};

export const getUsersByAge = async (age) => {
  const { data } = await gatewayAPI.get(`/api/usuarios/admin/users/by-age?age=${encodeURIComponent(age)}`);
  return data;
};
...
export const getPerformanceLatency = async () => {
  try {
    const { data } = await gatewayAPI.get('/api/usuarios/admin/metrics/latency');
    return data;
  } catch (err) {
    console.warn('Performance latency metric unavailable:', err.message);
    return null;
  }
};
...
export const getPerformanceThroughput = async () => {
  try {
    const { data } = await gatewayAPI.get('/api/usuarios/admin/metrics/throughput');
    return data;
  } catch (err) {
    console.warn('Performance throughput metric unavailable:', err.message);
    return null;
  }
};
...
export const getPerformanceErrors = async () => {
  try {
    const { data } = await gatewayAPI.get('/api/usuarios/admin/metrics/errors');
    return data;
  } catch (err) {
    console.warn('Performance errors metric unavailable:', err.message);
    return null;
  }
};

// ── Assistance service (→ gateway → ms_asistencia) ──────────────────────────
export const analyzeTeamAI = async (team) => {
  const { data } = await gatewayAPI.post('/api/asistencia/analyze/team', { team });
  return data;
};

export const recommendTeammateAI = async (team, top_n = 6) => {
  const { data } = await gatewayAPI.post('/api/asistencia/recommend/teammate', { team, top_n });
  return data;
};

export const recommendBuildAI = async (name) => {
  const { data } = await gatewayAPI.post('/api/asistencia/recommend/build', { name });
  return data;
};

// ── Monte Carlo simulation (→ gateway → ms_montecarlo) ───────────────────────
export const simulateBattle = async (payload) => {
  const { data } = await gatewayAPI.post('/api/montecarlo/simulate', payload);
  return data;
};

export const persistBestConfiguration = async (payload) => {
  const { data } = await gatewayAPI.post('/api/montecarlo/persist_best', payload);
  return data;
};

// ── Data loading (→ gateway → ms_carga_api) ─────────────────────────────────
export const loadExternalData = async (url) => {
  const { data } = await gatewayAPI.post('/api/carga/load', { url });
  return data;
};
