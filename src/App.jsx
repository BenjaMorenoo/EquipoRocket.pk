...
function PlaceholderPage({ title, icon, description }) {
  return (
    <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '32px', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 12px' }}>
        {title}
      </h1>
...
function AppContent() {
  ...
  const handleNavigate = (page) => { setCurrentPage(page); if (page !== 'builder') setEditingTeam(null); };
  const handleNavigateToBuilder = (team = null) => { setEditingTeam(team); setCurrentPage('builder'); };
  const handleSaveTeam = async (teamData) => { console.log('[TeamBuilder] Guardando equipo:', teamData); await new Promise((r) => setTimeout(r, 800)); setCurrentPage('teams'); };

  return (
    <div style={{ minHeight: '100vh' }} className="bg-grid">
      <Navbar currentPage={currentPage} onNavigate={handleNavigate} user={user} onLogout={logout} />
      <main style={{ position: 'relative', zIndex: 1 }}>
        ...
        {currentPage === 'ranking' && <PlaceholderPage title="Rankings" icon="🏆" description="Los equipos más usados en la meta competitiva y los mejores jugadores." />}
      </main>
    </div>
  );
}
...
function AuthGate() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
      <div style={{ width: '44px', height: '44px', borderRadius: '50%', border: '3px solid var(--color-pk-border)', borderTopColor: 'var(--color-pk-red)', animation: 'spin 0.8s linear infinite' }} />
